...
import { 
  removeProductFromDynamicHomeSectionAction, 
  updateDynamicHomeSectionItemRankAction as updateDynamicHomeSectionRankAction, 
  addProductToDynamicHomeSectionAction,
  updateDynamicHomeSectionItemAction as updateDynamicHomeSectionPriceOverrideAction
...
interface DynamicHomeSectionsTableProps {
  categoryId: string;
  sectionId: string;
  title: string;
  description: string;
...
export function DynamicHomeSectionsTable({ categoryId, sectionId, title, description, items }: DynamicHomeSectionsTableProps) {
  ...
  const handleRemove = async (productId: string) => {
    ...
    try {
      const result = await removeProductFromDynamicHomeSectionAction(categoryId, sectionId, productId);
      if (result.success) {
  ...
  const handleUpdateRank = async (productId: string) => {
    try {
      const result = await updateDynamicHomeSectionRankAction(categoryId, sectionId, productId, newRank);
      if (result.success) {
        toast.success(result.message);
        setEditingRank(null);
        // Refresh the page to show updated data
  ...
  const handleUpdatePriceOverride = async (productId: string) => {
    try {
      ...
      const result = await updateDynamicHomeSectionPriceOverrideAction(
        categoryId, 
        sectionId, 
        productId, 
  ...
  const handleAddProductToSection = async (productId: string) => {
    ...
    try {
      ...
      const result = await addProductToDynamicHomeSectionAction(categoryId, sectionId, productId, 1);
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            {sectionId === 'flashSale' && <TableHead>Price Override</TableHead>}
        ...
        <TableBody>
          {items.length === 0 ? (
            <TableRow>
              <TableCell colSpan={sectionId === 'flashSale' ? 5 : 4} className="text-center py-8 text-muted-foreground">
