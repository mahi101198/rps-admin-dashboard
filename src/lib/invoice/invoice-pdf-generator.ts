...
export class InvoicePDFGenerator {
  ...
  private addTotalsSection(invoiceData: InvoiceData): void {
    ...
    // Use either new or old variable names for flexibility
    const taxAmount = invoiceData.taxAmount || invoiceData.tax;
    const shippingCost = invoiceData.shippingCost || invoiceData.shipping;
    const discountAmount = invoiceData.discountAmount || invoiceData.discount;
    ...
    if (taxAmount > 0) {
      ...
      this.pdf.text(`₹${taxAmount.toFixed(2)}`, valueX, currentLineY, { align: 'right' });
      currentLineY += 5;
    }
    ...
    if (shippingCost > 0) {
      ...
      this.pdf.text(`₹${shippingCost.toFixed(2)}`, valueX, currentLineY, { align: 'right' });
      currentLineY += 5;
    }
    ...
    if (discountAmount > 0) {
      ...
      this.pdf.text(`-₹${discountAmount.toFixed(2)}`, valueX, currentLineY, { align: 'right' });
      currentLineY += 5;
    }
    ...
    this.pdf.text(`₹${totalAmount.toFixed(2)}`, valueX, currentLineY + 3, { align: 'right' });
